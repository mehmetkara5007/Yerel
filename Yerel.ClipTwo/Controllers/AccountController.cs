    ...
    public class AccountController : BaseController
    {
        ...
        public ActionResult Login(User user, string returnUrl)
        {
            if (user.Name == "Visma" & user.Password == "123123")
            {
                ...
                //request.AddHeader("Authorization", "Bearer {0}" + response.Data.Token);


                //  var dataList = client.Execute(request);

                InfoNotification("Your account successfully activated for data integration");
